                    ...
                    foreach ($datos1 as $row1) {
                        ...
                        if ($row1['CONTADOR'] != null) {
                            ...
                            foreach ($datos as $row) {
                                $insBeanPagination->setCountFilter($row['CONTADOR']);
                                if ($row['CONTADOR'] > 0) {
                                    $stmt = $conexion->prepare("SELECT b.*,s.nombre AS nombre_subtitulo,s.idsubtitulo FROM `recurso` AS b inner join `subtitulo` AS s ON s.codigo_subtitulo=b.codigo_subtitulo  WHERE(b.codigo_subtitulo <=? and b.disponible=1) or (b.codigo_subtitulo =? and b.disponible=0) ORDER BY  b.codigo_subtitulo  ASC LIMIT ?,?");
                            ...
                            $stmt->bindValue(":IDlibro", "L01.N01.", PDO::PARAM_STR);
                            $stmt->execute();
                            $variable = $stmt->fetchAll();
                            foreach ($variable as $row4) {
