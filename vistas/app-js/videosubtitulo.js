$(document).ready(function () {
    ...
    $("#btnAbrirvideos").click(function () {
        ...
        $("#imagePreview").html("");
        $("#videoPreview").html("");
        $("#Subcapitulo-reg  option").attr("selected", false);
        $("#Subcapitulo-reg  option[value=" + 0 + "]").attr("selected", true);
        $("#tituloModalManvideos").html("REGISTRAR VIDEO PARA EL SUBTITULO");
    ...
    $(".FormularioAjax").submit(function (e) {
        ...
        if (file2 && $("#Subcapitulo-reg option:selected").val() != 0) {
            $("#cargarpagina").html(ajax_load);
            ProcesarAjax(metodo, formdata);
        } else {
            if (tipo == "save") {
                if ($("#Subcapitulo-reg option:selected").val() == 0) {
                    swal(
                        "Ocurrió un error inesperado",
                        "Por favor debes seleccionar una SubTitulo",
                        "error"
                    );
                } else {
                    swal(
    ...
    function listar(paginas, registrototal) {
        $.ajax({
          ...
          <td class="text-center">${videos[key].subtitulo_codigosubtitulo}</td>
    ...
    function addEventsButtonsvideos() {
        $(".editar-videos").each(function (index, value) {
            $(this).click(function () {
                ...
                $("#ActualSubcapitulo-reg").val(rowvalue[indice - 1][1]);
                ...
                $(
                    "#Subcapitulo-reg option[value='" + rowvalue[indice - 1][1] + "']"
                ).attr("selected", true);
