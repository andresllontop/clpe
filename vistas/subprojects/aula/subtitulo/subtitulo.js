...
function processAjaxSubtitulo() {
	...
	switch (beanRequestSubtitulo.operation) {
		...
		case 'anteriorleccion':
			parameters_pagination += '?subtitulo=' + subtituloSelectedAll.codigo;
			break;
	...
	$.ajax({
		url:
			getHostAPI() +
			beanRequestSubtitulo.entity_api +
			'/' +
			beanRequestSubtitulo.operation +
			parameters_pagination,
		type: beanRequestSubtitulo.type_request,
		headers: {
			Authorization:
				'Bearer ' +
				Cookies.get('clpe_token') +
				(Cookies.get('clpe_libro') == undefined
					? ''
					: ' Clpe ' + Cookies.get('clpe_libro')),
		},
		data: form_data,
		cache: false,
		contentType:
				...
				if (beanCrudResponse.messageServer.toLowerCase() == 'ok') {
					...
					if (beanRequestSubtitulo.operation == 'updatestado') {
						...
						if (beanCrudResponse.beanPagination !== null) {
							...
							processAjaxSubtituloTitulo(
								document.querySelector('#bodySubtitulo-titulo-cuestionario')
							);
						}
					}
					...
					if (beanRequestSubtitulo.operation == 'updatestado') {
						...
						if (beanCrudResponse.beanPagination !== null) {
							...
							if (beanRequestSubtitulo.operation != 'anteriorleccion') {
								...
								subtituloSelectedInicial = beanPaginationSubtitulo.list[0];
								processAjaxSubtituloTitulo(
									document.querySelector('#bodySubtitulo-titulo')
								);
							} else {
								if (beanPaginationSubtitulo.countFilter > 0) {
									if (beanPaginationLeccion != undefined) {
										//cuestionario view
										addClass(
											document.querySelector('.container-flat-form')
												.parentElement,
											'd-none'
										);
										...
										listaSubtitulo(beanPaginationSubtitulo);
										if (
											subtituloSelectedInicial.subTitulo.codigo ==
											subtituloSelectedAll.codigo
										) {
											removeClass(
												document.querySelector('.container-flat-form')
													.parentElement,
									...
									showAlertTopEnd(
										'info',
										'No se encuentra la lección disponible',
										''
									);
								}
							}
		...
		.done(function (beanCrudResponse) {
			if (beanCrudResponse.beanPagination !== null) {
               ...
               donut_chart.setData([{ label: Math.round((beanCrudResponse.beanPagination.countFilter) * 100 / (totalTareas() - beanCrudResponse.beanPagination.countFilter)) + "%", value: 100 - Math.round((beanCrudResponse.beanPagination.countFilter) * 100 / (totalTareas() - beanCrudResponse.beanPagination.countFilter)) }, {
						...
						datasets: [
							{
								data: [
									Math.round(
										(beanCrudResponse.beanPagination.countFilter * 100) /
											parseInt(beanPaginationSubtituloTitulo.countFilter)
									),
									100 -
										Math.round(
											(beanCrudResponse.beanPagination.countFilter * 100) /
				...
				document.querySelector('#countRealizadas').innerHTML =
					beanCrudResponse.beanPagination.countFilter;
				document.querySelector('#countFaltantes').innerHTML =
					parseInt(beanPaginationSubtituloTitulo.countFilter) -
					beanCrudResponse.beanPagination.countFilter;
				document.querySelector('#countPorcentaje').innerHTML =
					Math.round(
						(beanCrudResponse.beanPagination.countFilter * 100) /
							parseInt(beanPaginationSubtituloTitulo.countFilter)
					) + '%';
...
function processAjaxSubtituloTitulo(
	documentId = document.querySelector('#bodySubtitulo-titulo')
) {
		...
		.done(function (beanCrudResponse) {
			...
			if (beanCrudResponse.messageServer !== null) {
				...
				if (beanCrudResponse.beanPagination !== null) {
					...
					beanPaginationSubtituloTitulo.list.forEach((recurso) => {
					...
					if (documentId == document.querySelector('#bodySubtitulo-titulo')) {
						...
						listaSubtituloTituloCuestionario(
							beanPaginationSubtituloTitulo,
							documentId
						);
					}
					processAjaxTarea();
...
function addSubtitulo(subtitulo = undefined) {
	...
	document.querySelector('#txtCodigoSubtitulo').value =
		subtitulo == undefined ? '' : subtitulo.codigo;

	document.querySelector('#txtNombreSubtitulo').value =
		subtitulo == undefined ? '' : subtitulo.nombre;
}
...
function listaSubtitulo(beanPagination) {
	...
	document.querySelector('.dt-titulo').innerHTML =
		'"' + subtituloSelected.titulo.nombre + '"';
	document.querySelector('.dt-subtitulo').innerHTML = subtituloSelected.nombre;
	document.querySelector(
		'.dt-subtitulo-pdf'
	).innerHTML = `<button type="button" class="btn btn-light"><img style="width:35px; height:35px;"src="${getHostFrontEnd()}vistas/assets/img/pdf.png" alt="Subtitulo"><figcaption  class="ml-1">Descargar Lección</figcaption> </button>`;
...
function listaSubtitulotoCuestionario(beanPagination) {
	...
	document
		.querySelector('#titleManRespuesta')
	...
	document.querySelector('.dt-titulo-cuest').innerHTML =
		'"' + subtituloSelected.titulo.nombre + '"';
	document.querySelector('.dt-subtitulo-cuest').innerHTML =
		subtituloSelected.nombre;
	...
	document.querySelectorAll('.descargar-archivo').forEach((btn) => {
		...
		btn.onclick = function () {
			...
			downloadURL(
				getHostFrontEnd() +
					'adjuntos/archivos/' +
					subtituloSelected.titulo.libro.codigo +
					'/' +
					subtituloSelected.titulo.codigo +
					'/PDF/' +
					subtituloSelected.pdf
			);
			document.querySelector('#descargarPdf').href =
				getHostFrontEnd() +
				'adjuntos/archivos/' +
				subtituloSelected.titulo.libro.codigo +
				'/' +
...
function findByVideoSubtitulo(idsubtitulo) {
	return beanPaginationSubtitulo.list.find((Subtitulo) => {
...
var validarDormularioSubtitulo = () => {
	...
	if (beanRequestSubtitulo.operation == 'add') {
		...
		if (
			!(
				document.querySelector('#txtImagenSubtitulo').files[0].type ==
					'image/png' ||
				document.querySelector('#txtImagenSubtitulo').files[0].type ==
					'image/jpg' ||
				document.querySelector('#txtImagenSubtitulo').files[0].type ==
			...
			showAlertTopEnd(
				'info',
				'Formato Incorrecto',
				'Ingrese tipo de arhivo Imagen => png,jpg,jpeg'
			);
			return false;
		}
		//menor a   1700 KB
		if (
			document.querySelector('#txtImagenSubtitulo').files[0].size >
			1700 * 1024
		) {
			showAlertTopEnd(
				'info',
				'Tamaño excedido',
				'el tamaño del archivo tiene que ser menor a 1700 KB'
			);
			return false;
		}
		...
		$(this)
			...
			.each(function () {
				...
				if ($(this).hasClass('active'))
					$(a).append(
						'<i class="fa fa-chevron-down" style="line-height: 25px;"></i>'
					);
				else
					$(a).append(
						'<i class="fa fa-chevron-right" style="line-height: 25px;"></i>'
					);
...
function listaSubtituloTitulo(
	...
	beanPagination.list.forEach((subtitulo) => {
		if (subtitulo.disponible == undefined) {
                ...
                <article>
                    <div class="btn-light form-row pl-4">
                        <p class="col-1" style="max-width: 1.7em;">
                            <input type="checkbox" ${
															subtitulo.codigo <
															(beanPaginationSubtitulo == undefined
																? beanPaginationLeccion.list[0].subtitulo.codigo
																: beanPaginationSubtitulo.list[0].subTitulo
																		.codigo)
																? 'checked'
																: ''
														} class="aula-cursor-mano" style="
            ...
            <li class="${
							(beanPaginationSubtitulo == undefined
								? beanPaginationLeccion.list[0].subtitulo.codigo.split('.')[0] +
								  '.' +
								  beanPaginationLeccion.list[0].subtitulo.codigo.split('.')[1] +
								  '.' +
								  beanPaginationLeccion.list[0].subtitulo.codigo.split('.')[2]
								: beanPaginationSubtitulo.list[0].subTitulo.titulo.codigo) ==
							subtitulo.titulo.codigo
								? 'active'
								: ''
						}" style="border-bottom: 1px solid #dedfe0;">
            <span><a href="#" class="text-udemy f-weight-700 py-2" style="line-height: 24px;">Capítulo ${
							parseInt(subtitulo.codigo.split('.')[2].replace('N', '')) +
							' : ' +
							subtitulo.titulo.nombre
						}</a></span>
            <article ${
							subtitulo.codigo >
							(beanPaginationSubtitulo == undefined
								? beanPaginationLeccion.list[0].subtitulo.codigo
								: beanPaginationSubtitulo.list[0].subTitulo.codigo)
								? ''
								: " idsubtitulo='" +
								  subtitulo.idsubTitulo +
								  "' class='seleccionar-subtitulo aula-cursor-mano'"
						}>
                <div class="btn-light form-row pl-4 ${
									beanPaginationSubtitulo == undefined
										? ''
                    ...
                    <p class="col-sm-6 col-11 py-1">
                        <span class="text-udemy f-14">Subtítulo ${
													contadorsubtitulo++ + ' : ' + subtitulo.nombre
												}</span>
                        <br>
                        ...
                        ${
													recurso == undefined
														? ''
														: '<a href="recursos" class="p-1"><button type="button" class="btn btn-outline-info py-0 mx-2"><i class="zmdi zmdi-folder-outline mr-1"></i> Recursos</button></a>'
												}
            ...
            `;
                        ...
                        <div class="btn-light form-row pl-4 ${
                            ...
                            <p class="col-1" style="max-width: 1.7em;">
                                <input type="checkbox" ${
																	subtitulo.codigo <
																	(beanPaginationSubtitulo == undefined
																		? beanPaginationLeccion.list[0].subtitulo
																				.codigo
																		: beanPaginationSubtitulo.list[0].subTitulo
																				.codigo)
																		? 'checked'
                    ...
                    <li class="${
											(beanPaginationSubtitulo == undefined
												? beanPaginationLeccion.list[0].subtitulo.codigo.split(
														'.'
												  )[0] +
												  '.' +
												  beanPaginationLeccion.list[0].subtitulo.codigo.split(
												...
												: beanPaginationSubtitulo.list[0].subTitulo.titulo
														.codigo) == subtitulo.titulo.codigo
												? 'active'
												: ''
										}" style="border-bottom: 1px solid #dedfe0;">
                    <span><a href="#" class="text-udemy f-weight-700 py-2" style="line-height: 24px;">Capítulo ${
											parseInt(
												subtitulo.codigo.split('.')[2].replace('N', '')
											) +
											' : ' +
											subtitulo.titulo.nombre
										}</a></span>
			...
			if (
				beanPaginationSubtituloTitulo.list[
					beanPaginationSubtituloTitulo.list.length - 1
				].codigo == subtitulo.codigo
			) {
				row += `
    </li>
...
function listaSubtituloTituloCuestionario(
	beanPagination,
	documentId = document.querySelector('#bodySubtitulo-titulo')
) {
	let row = '',
		...
		if (subtitulo.disponible == undefined) {
			if (subtitulo.titulo == '') {
				if (subtitulo.estado == 1) {
					row += `
                <article ${
									beanPaginationLeccion.list[0].test.tipo == 1
										? beanPaginationLeccion.list[0].subtitulo.codigo.split(
												'.'
										  )[0] +
												'.' +
										  ...
										  subtitulo.codigo.split('.')[0] +
												'.' +
												subtitulo.codigo.split('.')[1] +
												'.' +
												subtitulo.codigo.split('.')[2]
											? "class='ver-cuestionario'"
											: ''
										: ''
								} >
                    <div class="btn-light form-row pl-4 ${
                ...
                `;
					...
					row += `
                <article ${
									beanPaginationLeccion.list[0].test.tipo == 2
										? subtitulo.codigo.split('.')[0] +
												'.' +
												subtitulo.codigo.split('.')[1] +
												'.' +
                    ...
                    <div class="btn-light form-row pl-4 ${
											beanPaginationLeccion.list[0].subtitulo.titulo ==
											undefined
												? ''
												: beanPaginationLeccion.list[0].subtitulo.codigo ==
												  subtitulo.codigo
...
function addEventLeccion() {
	document.querySelectorAll('.seleccionar-subtitulo').forEach((btn) => {
		//AGREGANDO EVENTO CLICK
		btn.onclick = function () {
			subtituloSelectedAll = findBySubtitulo(btn.getAttribute('idsubtitulo'));
	...
	document.querySelectorAll('.ver-cuestionario').forEach((btn) => {
		...
		btn.onclick = function () {
			...
			removeClass(
				document.querySelector('#titleManRespuesta').parentElement
					.parentElement,
				'd-none'
			);
...
function findBySubtitulo(idblog) {
	return beanPaginationSubtituloTitulo.list.find((Blog) => {
...
function totalTareas() {
	let contador = 0;
	beanPaginationSubtituloTitulo.list.find((Blog) => {
...
function findByRecurso(codigo) {
	return beanPaginationRecurso.find((recurso) => {
