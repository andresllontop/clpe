...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (GETsearch.split("/").length == 5) {
        if (/^[0-9.]*$/.test(GETsearch.split("/")[4])) {
            comprarSelected = { "idcurso": GETsearch.split("/")[4] };
        } else {
            window.location.href = getHostFrontEnd() + "aula/matricula";
        }
    } else {
        window.location.href = getHostFrontEnd() + "aula/matricula";
    }
...
function getCliente() {
    // circleCargando.containerOcultar = $(document.querySelector("#msform").firstElementChild);
    // circleCargando.container = $(document.querySelector("#msform"));
    // circleCargando.createLoader();
    //circleCargando.toggleLoader("show");
    fetch(getHostAPI() + "cliente/obtener", {
        headers: {
            "Content-Type": 'application/json; charset=UTF-8',
        ...
        .then(beanCrudResponse => {
            ...
            if (beanCrudResponse.beanPagination !== null) {
                if (beanCrudResponse.beanPagination.countFilter > 0) {
                    document.querySelector("#txtnombreCuenta").value = beanCrudResponse.beanPagination.list[0].nombre;
                    document.querySelector("#txtapellidoCuenta").value = beanCrudResponse.beanPagination.list[0].apellido;
                    document.querySelector("#txtprofesionCuenta").value = beanCrudResponse.beanPagination.list[0].ocupacion;
                    document.querySelector("#txtcountryCuenta").value = beanCrudResponse.beanPagination.list[0].pais;
                    document.querySelector("#txtTelefonoCuenta").value = beanCrudResponse.beanPagination.list[0].telefono;
                    document.querySelector("#txtemailCuenta").value = beanCrudResponse.beanPagination.list[0].cuenta.email;
                    document.querySelector("#txtpassCuenta").value = beanCrudResponse.beanPagination.list[0].cuenta.clave;
                    document.querySelector("#txtpassconfirmCuenta").value = beanCrudResponse.beanPagination.list[0].cuenta.clave;
                    ///
                    document.querySelector("#txtnombreCuentaOtro").value = beanCrudResponse.beanPagination.list[0].nombre;
                    document.querySelector("#txtapellidoCuentaOtro").value = beanCrudResponse.beanPagination.list[0].apellido;
                    document.querySelector("#txtprofesionCuentaOtro").value = beanCrudResponse.beanPagination.list[0].ocupacion;
                    document.querySelector("#txtcountryCuentaOtro").value = beanCrudResponse.beanPagination.list[0].pais;
                    document.querySelector("#txtTelefonoCuentaOtro").value = beanCrudResponse.beanPagination.list[0].telefono;
                    document.querySelector("#txtemailCuentaOtro").value = beanCrudResponse.beanPagination.list[0].cuenta.email;
                    document.querySelector("#txtpassCuentaOtro").value = beanCrudResponse.beanPagination.list[0].cuenta.clave;
                    document.querySelector("#txtpassconfirmCuentaOtro").value = beanCrudResponse.beanPagination.list[0].cuenta.clave;
...
function processAjaxRegisterCompra(form_data, json = undefined) {
    $.ajax({
        ...
        setTimeout(function () { $('#modalCargandoCompra').modal('hide'); }, 500);
        if (beanCrudResponse.messageServer != undefined) {
            if (beanCrudResponse.messageServer.toLowerCase() == 'ok' || beanCrudResponse.messageServer.toLowerCase() == 'new libro add') {
                ...
                if (beanRequestComprar.operation == 'charge') {
                    setTimeout(function () { location.reload(); }, 8000);
                }
            }
        } else {
