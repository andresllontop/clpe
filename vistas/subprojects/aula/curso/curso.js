		...
		.done(function (beanCrudResponse) {
			if (beanCrudResponse.beanPagination !== null) {
				...
				if (parseInt(beanPaginationCursoUnico.countFilter) == 1) {
					console.log(beanPaginationCursoUnico);
...
function listaCurso(beanPagination) {
	let row = '';
	document.querySelector('#countCurso').innerHTML =
		'Tenemos ' + beanPagination.countFilter + 'cursos que te van a gustar';
	beanPagination.list.forEach((curso) => {
		row += `
    <div class="item block ver-detalle mx-auto" codigocurso="${
			curso.libro.codigo
		}">
    <div class="thumbs-wrapper">
        <div class="thumbs">
            <img style="width: 100%;height: 315px;"
                src="${getHostFrontEnd()}adjuntos/libros/${
			curso.libro.imagen
		}" />
