...
document.addEventListener('DOMContentLoaded', function () {
	...
	if (Cookies.get('clpe_niubiz_date') != undefined) {
		if (
			new Date(parseInt(Cookies.get('clpe_niubiz_date'))) -
	...
	if (Cookies.get('clpe_niubiz') != undefined) {
		...
		if (sesionCompra != undefined) {
			if (sesionCompra.nombre != undefined) {
				...
				document.querySelector('#txtapellidoCuenta').value =
					sesionCompra.apellido;
				document.querySelector('#txtprofesionCuenta').value =
					sesionCompra.profesion;
				document.querySelector('#txtTelefonoCuenta').value =
					sesionCompra.telefono;
				...
				document.querySelector('#txtpassconfirmCuenta').value =
					sesionCompra.pass;
			}
		}
	}
	...
	if (Cookies.get('clpe_empresa_compra') == undefined) {
		circleCargando.containerOcultar = $(
			document.querySelector('#precioCompra')
		);
		circleCargando.container = $(
			document.querySelector('#precioCompra').parentElement
		);
		...
		fetch(getHostAPI() + 'empresa/curso-' + comprarSelected.idcurso, {
			...
			.then((json) => {
				...
				if (json.messageServer == 'ok') {
					...
					Cookies.set(
						'clpe_niubiz_date',
						parseInt(new Date().getTime()) + parseInt(20 * 60 * 1000)
					);
					requestEmpresa(json);
				} else {
					window.location.href = getHostFrontEnd() + 'matricula';
	...
	document.getElementsByName('radioTipoComunicacionCuenta').forEach((btn) => {
		...
		btn.onchange = function () {
			if (btn.checked == true && parseInt(btn.value) == parseInt(4)) {
				removeClass(
					document.querySelector('#txtCodigoVendedorCuenta').parentElement,
					'd-none'
				);
	...
	document.querySelector('#txtcountryCuenta').onchange = function () {
		paisSelectedCompra = findByPaisesCompra(
			document.querySelector('#txtcountryCuenta').value
		);
		document.querySelector(
			'.paises-cuenta'
		).innerHTML = `<i class="flag-icon ${paisSelectedCompra.icon} mr-2"></i>${paisSelectedCompra.nombre} (+${paisSelectedCompra.codigo})`;
	};

	$('#txtTelefonoCodigoCuenta > li').click(function (btn) {
		document.querySelector('.paises-cuenta').innerHTML =
	...
	$('#txtTelefonoCodigoCuentaOtro > li').click(function (btn) {
		document.querySelector('.paises-cuentaOtro').innerHTML =
			btn.currentTarget.innerHTML;
	});

	document.querySelector('#txtcountryCuentaOtro').onchange = function () {
		paisSelectedCompra = findByPaisesCompra(
			document.querySelector('#txtcountryCuentaOtro').value
		);
		document.querySelector(
			'.paises-cuentaOtro'
		).innerHTML = `<i class="flag-icon ${paisSelectedCompra.icon} mr-2"></i>${paisSelectedCompra.nombre} (+${paisSelectedCompra.codigo})`;
	...
	$('#registerOtroMedio').submit(function (event) {
		if (validarFormularioRegisterOtroCompra()) {
			let radioButTrat = document.getElementsByName(
				'radioTipoComunicacionOtro2'
			);
			let valorRadio = 0;
			...
			let jso = {
				telefono:
					paisSelectedCompra.codigo +
					document.querySelector('#txtTelefonoCuentaOtro').value,
				...
				vendedor:
					document.querySelector('#txtCodigoVendedorClienteOtro2').value == ''
						? null
						: document.querySelector('#txtCodigoVendedorClienteOtro2').value,
...
var findByPaisesCompra = (nombre) => {
	return stateCompra.find((detalle) => {
		if (
			removeAccents(nombre).toLocaleLowerCase() ==
			removeAccents(detalle.nombre).toLocaleLowerCase()
		) {
			return detalle;
		}
	});
...
function requestEmpresa(json) {
	...
	let paises = "<option value=''>[Pais]</option>",
		codigos = '';
	...
	if (json.pais.currencySymbol != undefined) {
		document.querySelector('#precioCompra').innerHTML =
			`<small class="mr-1">${json.pais.currencySymbol}</small>` + json.precio;
	}

	if (!(json.pais.currencyId == 'USD')) {
		document.querySelector('#precioCompraUSD').innerHTML =
			`= <small class="mr-1">$</small>` +
			json.precio_USD +
			`<small class="mx-1">USD</small>` +
	...
	document
		.querySelector('#emailComprar')
		.setAttribute('href', 'mailto:' + json.email);
	...
	stateCompra.forEach((country) => {
		if (
			removeAccents(country.nombre).toLocaleLowerCase() ==
			removeAccents(json.pais.name).toLocaleLowerCase()
		) {
			...
			if (json.pais.currencySymbol != undefined) {
				document.querySelector(
					'#precioCompra'
				).innerHTML = `<small class="mr-1">${json.pais.currencySymbol}</small>${json.precio}<i class="flag-icon ${country.icon} mx-2"></i>`;
			}
		} else {
...
function eventoCompra() {
	document.querySelector('#span-ocultar-passCuenta2').onclick = () => {
		if (
			document
				.querySelector('#span-ocultar-passCuenta2')
				.firstElementChild.className.includes('zmdi-eye-off')
		) {
			document
				.querySelector('#txtpassconfirmCuenta')
	...
	document.querySelector('#span-ocultar-passCuenta1').onclick = () => {
		if (
			document
				.querySelector('#span-ocultar-passCuenta1')
				.firstElementChild.className.includes('zmdi-eye-off')
		) {
	...
	document.querySelector('#span-ocultar-passCuentaOtro2').onclick = () => {
		if (
			document
				.querySelector('#span-ocultar-passCuentaOtro2')
				.firstElementChild.className.includes('zmdi-eye-off')
		) {
			document
				.querySelector('#txtpassconfirmCuentaOtro')
				.setAttribute('type', 'password');
			document
				.querySelector('#span-ocultar-passCuentaOtro2')
				.firstElementChild.classList.remove('zmdi-eye-off');
	...
	document.querySelector('#span-ocultar-passCuentaOtro1').onclick = () => {
		if (
			document
				.querySelector('#span-ocultar-passCuentaOtro1')
				.firstElementChild.className.includes('zmdi-eye-off')
		) {
			document
				.querySelector('#txtpassCuentaOtro')
				.setAttribute('type', 'password');
			document
				.querySelector('#span-ocultar-passCuentaOtro1')
				.firstElementChild.classList.remove('zmdi-eye-off');
	...
	amex.attr(
		'src',
		getHostFrontEnd() + 'vistas/subprojects/publico/comprar/images/amex.jpg'
	);
	visa.attr(
		'src',
		getHostFrontEnd() + 'vistas/subprojects/publico/comprar/images/visa.jpg'
	);
	mastercard.attr(
		'src',
		getHostFrontEnd() +
			'vistas/subprojects/publico/comprar/images/mastercard.jpg'
	);
	$('.bcp').attr(
		'src',
		getHostFrontEnd() + 'vistas/subprojects/publico/comprar/images/bcp.png'
	);
	$('.banco-nacion-peru').attr(
		'src',
		getHostFrontEnd() +
			'vistas/subprojects/publico/comprar/images/banco-nacion-peru.png'
	);
	$('.caja-piura').attr(
		'src',
		getHostFrontEnd() +
			'vistas/subprojects/publico/comprar/images/caja-piura.jpg'
	);
	...
	document.querySelector('#compraRadios2').onclick = function () {
		if (this.checked) {
			...
			document
				.getElementsByName('radioTipoComunicacionOtro2')
				.forEach((btn) => {
					//AGREGANDO EVENTO CLICK
					btn.onchange = function () {
						if (btn.checked == true && parseInt(btn.value) == parseInt(4)) {
							removeClass(
								document.querySelector('#txtCodigoVendedorClienteOtro2')
									.parentElement,
								'd-none'
							);
						} else if (btn.checked == true) {
...
function sesionNiubiz() {
	...
	circleCargando.containerOcultar = $(
		document.querySelector('#txtNumeroTarjeta').parentElement.parentElement
	);
	circleCargando.container = $(
		document.querySelector('#txtNumeroTarjeta').parentElement.parentElement
			.parentElement
 ...
 */
	...
	fetch(
		getHostAPI() +
			'niubiz/sesion' +
			'?amount=' +
			parseFloat(beanPaginationFooter.precio_USD) +
			'&channel=web',
		{
			headers: {
				'Content-Type': 'application/json; charset=UTF-8',
...
function payNiubiz() {
	...
	let json = {
		...
		recurrence: false,
		vendedor:
			document.querySelector('#txtCodigoVendedorCuenta').value == ''
				? null
				: document.querySelector('#txtCodigoVendedorCuenta').value,
		tipomedio: valorRadio,
	};
	...
	window.payform
		.createToken([window.cardNumber, window.cardExpiry, window.cardCvv], {
			name: json.nombre,
...
var validarFormularioRegisterCompra = () => {
	...
	if (letra != undefined) {
		if (letra.value == '') {
			showAlertTopEnd(
				'info',
				'Formato Incorrecto',
				'Por favor ingrese datos al campo' + letra.labels[0].innerText
			);
		} else {
			showAlertTopEnd(
				'info',
	...
	let numero = numero_campo(document.querySelector('#txtTelefonoCuenta'));
	...
	if (numero != undefined) {
		if (numero.value == '') {
			showAlertTopEnd(
				'info',
				'Formato Incorrecto',
				'Por favor ingrese datos al campo teléfono' + numero.labels[0].innerText
			);
		} else {
			showAlertTopEnd(
				'info',
	...
	if (
		parseInt(document.querySelector('#txtTelefonoCuenta').value).length > 20
	) {
		showAlertTopEnd(
			'info',
			'Formato Incorrecto',
			'Por favor ingrese sólo números menor a 20 dígitos'
		);
		return false;
	}
	let email = email_campo(document.querySelector('#txtemailCuenta'));
	...
	document.querySelector('#txtpassCuenta').value = limpiar_campo(
		document.querySelector('#txtpassCuenta').value
	);
	if (
		limpiar_campo(document.querySelector('#txtpassconfirmCuenta').value) !=
		document.querySelector('#txtpassCuenta').value
	) {
		swal({
			title: 'Formato Incorrecto',
...
var validarFormularioRegisterCompraPersonales = () => {
	...
	removeClass(
		document.querySelector('#txtnombreCuenta').labels[0],
		'text-danger-compra font-weight-400'
	);
	removeClass(
		document.querySelector('#txtnombreCuenta'),
		'border-danger-compra'
	);
	removeClass(
		document.querySelector('#txtapellidoCuenta').labels[0],
		'text-danger-compra font-weight-400'
	);
	removeClass(
		document.querySelector('#txtapellidoCuenta'),
	...
	removeClass(
		document.querySelector('#txtprofesionCuenta').labels[0],
		'text-danger-compra font-weight-400'
	);
	removeClass(
		document.querySelector('#txtprofesionCuenta'),
		...
		if (letra.value == '') {
			showAlertTopEnd(
				...
				'Por favor ingrese datos al campo ' +
					letra.labels[0].innerText.toLowerCase()
			);
		} else {
			showAlertTopEnd(
				'info',
				'Formato Incorrecto',
				'Por favor ingrese sólo letras, al campo ' +
					letra.labels[0].innerText.toLowerCase()
			);
		}
	...
	if (numero != undefined) {
		...
		if (numero.value == '') {
			showAlertTopEnd(
				...
				'Formato Incorrecto',
				'Por favor ingrese datos al campo ' +
					numero.labels[0].innerText.toLowerCase()
			);
		} else {
			showAlertTopEnd(
				'info',
		...
		removeClass(
			document.querySelector('#txtTelefonoCuenta').labels[0],
			'text-danger-compra font-weight-400'
		);
		removeClass(
	...
	if (document.querySelector('#txtcountryCuenta').value == '') {
		addClass(
			document.querySelector('#txtcountryCuenta').labels[0],
			'text-danger-compra font-weight-400'
		);
		addClass(
			document.querySelector('#txtcountryCuenta'),
			'border-danger-compra'
		);
		swal({
			title: 'Formato Incorrecto',
...
var validarFormularioRegisterCompraCuenta = () => {
	...
	if (email != undefined) {
		...
		removeClass(
			document.querySelector('#txtemailCuenta').labels[0],
			'text-danger-compra font-weight-400'
		);
		removeClass(
...
var validarFormularioRegisterOtroCompra = () => {
	...
	let numero = numero_campo(document.querySelector('#txtTelefonoCuentaOtro'));
	...
	if (
		parseInt(document.querySelector('#txtTelefonoCuentaOtro').value).length > 20
	) {
		showAlertTopEnd(
			'info',
			'Formato Incorrecto',
	...
	let email = email_campo(document.querySelector('#txtemailCuentaOtro'));
	...
	document.querySelector('#txtpassCuentaOtro').value = limpiar_campo(
		document.querySelector('#txtpassCuentaOtro').value
	);
	if (
		limpiar_campo(document.querySelector('#txtpassconfirmCuentaOtro').value) !=
		document.querySelector('#txtpassCuentaOtro').value
	) {
		swal({
			title: 'Formato Incorrecto',
	...
	if (
		!(
			document.querySelector('#txtvoucherCuentaOtro').files[0].type ==
				'image/png' ||
			document.querySelector('#txtvoucherCuentaOtro').files[0].type ==
				'image/jpg' ||
			document.querySelector('#txtvoucherCuentaOtro').files[0].type ==
		...
		showAlertTopEnd(
			'info',
			'Formato Incorrecto',
			'Ingrese tipo de arhivo del voucher => png,jpg,jpeg'
		);
		return false;
	}
	//menor a   5 MB
	if (
		document.querySelector('#txtvoucherCuentaOtro').files[0].size >
		5 * 1024 * 1024
	) {
		showAlertTopEnd(
			'info',
			'Tamaño excedido',
			'el tamaño del archivo tiene que ser menor a 5 MB'
		);
		return false;
	}
...
function processAjaxRegisterCompra(form_data, json = undefined) {
	$.ajax({
		url:
			getHostAPI() +
			beanRequestComprar.entity_api +
			'/' +
			beanRequestComprar.operation,
		type: 'POST',
		data: form_data,
		cache: false,
		contentType: false,
			...
			xhr.upload.addEventListener(
				...
				function (evt) {
					if (evt.lengthComputable) {
						...
						$('.progress-bar-compra').text(
							Math.round(percentComplete * 100) + '%'
						);
						$('.progress-bar-compra').attr(
							'aria-valuenow',
							+Math.round(percentComplete * 100)
						);
		...
		.done(function (beanCrudResponse) {
			...
			if (beanCrudResponse.messageServer != undefined) {
				if (
					beanCrudResponse.messageServer.toLowerCase() == 'ok' ||
					beanCrudResponse.messageServer.toLowerCase() == 'new libro add'
				) {
					addClass(
						document.querySelector('.success-compra'),
						'active pulse-2 m-2 border'
					);
					document
						.querySelector('.success-compra')
						.setAttribute(
							'style',
							'background: white;border: 0 none;border-radius: 3px;box-shadow: 0 0 15px 1px rgba(0, 0, 0, 0.4);padding: 20px 30px;box-sizing: border-box;margin: 0 10%;'
						);
					if (document.querySelector('#compraRadios1').checked) {
						document.querySelector('.next').dispatchEvent(new Event('click'));
						document.querySelector(
							'.success-compra'
						).innerHTML = `<h2 class="text-center">COMPRA EXITOSA!</h2>
                    <p> N° de Pedido : ${
											beanCrudResponse.beanClass.nPedido
										} </p>
                    <p> Apellido y Nombre : ${
											beanCrudResponse.beanClass.nombre
										} </p>
                    <p> Fecha y Hora de Pedido : ${
											beanCrudResponse.beanClass.fecha
										} </p>
                    <p> Importe de la Transacción : ${
											beanCrudResponse.beanClass.importe
										} </p>
                    <p> Tipo de Moneda : ${
											beanCrudResponse.beanClass.tipoCurrency
										} </p>
                    <p> Producto : "TALLER DE LECTURA '${
											JSON.parse(Cookies.get('clpe_empresa_compra')).nombre
										}'"</p>
                    <p> Tarjeta : ${
											beanCrudResponse.beanClass.marcaTarjeta
										} </p>
                    <p> N° de Tarjeta : ${
											beanCrudResponse.beanClass.tarjeta
										} </p>
                    ...
                    <p>se envió a tu correo un código de verificación para que puedas acceder al curso.</p>`;
						...
						document.querySelector(
							'.success-compra'
						).innerHTML = `<h2 class="text-center">COMPRA EXITOSA!</h2><p>en un máximo de 24 horas el administrador de Club de Lectura para emprendedores verificará el depósito realizado, y enviará a tu correo un código de verificación para que puedas acceder al curso '${
							JSON.parse(Cookies.get('clpe_empresa_compra')).nombre
						}'</p>`;
					}
...
function paisArray() {
	stateCompra = new Array(
		{ codigo: '355', icon: 'flag-icon-al', nombre: 'Albania' },
		{ codigo: '376', icon: 'flag-icon-ad', nombre: 'Andorra' },
		{ codigo: '244', icon: 'flag-icon-ao', nombre: 'Angola' },
		{ codigo: '54', icon: 'flag-icon-ar', nombre: 'Argentina' },
		{ codigo: '374', icon: 'flag-icon-am', nombre: 'Armenia' },
		{ codigo: '61', icon: 'flag-icon-au', nombre: 'Australia' },
		{ codigo: '43', icon: 'flag-icon-at', nombre: 'Austria' },
		{ codigo: '973', icon: 'flag-icon-bh', nombre: 'Bahrain' },
		{ codigo: '880', icon: 'flag-icon-bd', nombre: 'Bangladesh' },
		{ codigo: '591', icon: 'flag-icon-bo', nombre: 'Bolivia' },
		{ codigo: '55', icon: 'flag-icon-br', nombre: 'Brazil' },
		{ codigo: '359', icon: 'flag-icon-bg', nombre: 'Bulgaria' },
		{ codigo: '1', icon: 'flag-icon-ca', nombre: 'Canada' },
		{ codigo: '236', icon: 'flag-icon-cf', nombre: 'Central African Republic' },
		{ codigo: '56', icon: 'flag-icon-cl', nombre: 'Chile' },
		{ codigo: '57', icon: 'flag-icon-co', nombre: 'Colombia' },
		{ codigo: '506', icon: 'flag-icon-cr', nombre: 'Costa Rica' },
		{ codigo: '53', icon: 'flag-icon-cu', nombre: 'Cuba' },
		{ codigo: '45', icon: 'flag-icon-dk', nombre: 'Dinamarca' },
		{ codigo: '1809', icon: 'flag-icon-do', nombre: 'Dominican Republic' },
		{ codigo: '593', icon: 'flag-icon-ec', nombre: 'Ecuador' },
		{ codigo: '503', icon: 'flag-icon-sv', nombre: 'El Salvador' },
		{ codigo: '001', icon: 'flag-icon-us', nombre: 'Estados Unidos' },
		{ codigo: '33', icon: 'flag-icon-fr', nombre: 'France' },
		{ codigo: '502', icon: 'flag-icon-gt', nombre: 'Guatemala' },
		{ codigo: '504', icon: 'flag-icon-hn', nombre: 'Honduras' },
		{ codigo: '39', icon: 'flag-icon-it', nombre: 'Italy' },
		{ codigo: '1876', icon: 'flag-icon-jm', nombre: 'Jamaica' },
		{ codigo: '81', icon: 'flag-icon-jp', nombre: 'Japan' },
		{ codigo: '52', icon: 'flag-icon-mx', nombre: 'México' },
		{ codigo: '505', icon: 'flag-icon-ni', nombre: 'Nicaragua' },
		{ codigo: '507', icon: 'flag-icon-pa', nombre: 'Panama' },
		{ codigo: '595', icon: 'flag-icon-py', nombre: 'Paraguay' },
		{ codigo: '51', icon: 'flag-icon-pe', nombre: 'Perú' },
		{ codigo: '351', icon: 'flag-icon-pt', nombre: 'Portugal' },
		{ codigo: '44', icon: 'flag-icon-gb', nombre: 'Reino Unido' },
		{ codigo: '7', icon: 'flag-icon-ru', nombre: 'Rusia' },
		{ codigo: '90', icon: 'flag-icon-tr', nombre: 'Turquía' },
		{ codigo: '598', icon: 'flag-icon-uy', nombre: 'Uruguay' },
		{ codigo: '58', icon: 'flag-icon-ve', nombre: 'Venezuela' },
		{ codigo: '260', icon: 'flag-icon-zm', nombre: 'Zambia' }
	);
}
