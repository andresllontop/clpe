...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (GETsearch.split("/").length == 5) {
    if (/^[0-9.]*$/.test(GETsearch.split("/")[4])) {
      ...
      circleCargando.containerOcultar = $(document.querySelector("#htmlCargandoMatricula"));
      circleCargando.container = $(document.querySelector("#htmlCargandoMatricula").parentElement);
      circleCargando.createLoader();
      circleCargando.toggleLoader("show");
      Promise.all([
        fetch(getHostAPI() + beanRequestCurso.entity_api + "/" + beanRequestCurso.operation +
          "?tipo=0&idcurso=" + cursoSelected.idcurso + "&pagina=1&registros=100", fetOptions),
...
function listaCurso(beanPagination) {
  ...
  let row1 = "", row3 = "", contador = 0,
    contadorTipo = findByBeneficioLibro(2), contadorDeg = 130, contadorDism = 18;
  ...
  beanPagination.list.forEach((curso) => {
    if (curso.precio == undefined) {
      if (curso.tipo == 1) {
        ...
        contadorDism += 2;
        document.querySelector(".page").innerHTML += `<li class="" data-next="${contadorDeg}" data-anterior="${contadorDism}" style="-webkit-transform: rotateY(-${contadorDism}deg);
        ...
        row3 += `<li class=" mb-1 ${contador == 0 ? "active pulse-2 m-2" : ""} border" style="border-color: #b21aff;"><span><a href="#">${curso.titulo}</a></span><article><p>${curso.detalle}</p></article></li>`;
        contador++;
      } else if (curso.tipo == 4) {

        document.querySelector('#txtImagenCurso').innerHTML = `<img class="w-100" src="${getHostFrontEnd() + "adjuntos/libros/" + curso.imagen}">`;
        document.querySelector('#txtObjetivoCurso').innerHTML = curso.detalle;
      }
    } else {

      if (document.querySelector('.precio-curso')) {
        document.querySelector('.precio-curso').innerHTML = `<span style="font-weight: 100;color: #8c8b8b;">$</span>${curso.precio}<span style="font-size: 25px;
  ...
  if (document.querySelector("li .page").lastElementChild) {
    document.querySelector("li .page").lastElementChild.classList.add("page-active");
  }
  eventoLibro();
  eventosAcordion();
          ...
          if (!$(parent).hasClass("active")) {
            $("ul.accordion li.active article").slideUp(
              ...
              function () {
                $(this)
                  .parent("li")
                  .removeClass("active pulse-2 m-2");
                $(this)
                  .siblings("span")
                  .children("a")
                  .children("i")
...
function eventoLibro() {
  ...
  document.querySelector("#VerNext").onclick = () => {
    ...
    if ($pageItem.previousElementSibling) {
      ...
      $pageItem.setAttribute("style", "-webkit-transform: rotateY(-" + otroDegra + "deg);-moz-transform: rotateY(-" + otroDegra + "deg);transform: rotateY(-" + otroDegra + "deg);background-color: white; color: transparent;-webkit-transition:-webkit-transform 1.5s,background-color .6s,color .6s;transition:transform 1.5s, background-color .6s,color .6s;-moz-transition:-moz-transform 1.5s, background-color .6s,color .6s");
  ...
  document.querySelector("#VerAnterior").onclick = () => {
    ...
    if ($pageItem.nextElementSibling) {
      ...
      $pageItem.nextElementSibling.setAttribute("style", "-webkit-transform: rotateY(-" + otroDegra + "deg);-moz-transform: rotateY(-" + otroDegra + "deg);transform: rotateY(-" + otroDegra + "deg); -webkit-transition:-webkit-transform 1.5s,color 5s;transition:transform 1.5s, color 5s;-moz-transition:-moz-transform 1.5s, color 5s");
      $pageItem.classList.remove("page-active");
