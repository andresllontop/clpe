...
document.addEventListener('DOMContentLoaded', function () {
    beanRequestPromotorNoticia.entity_api = 'noticias';
    beanRequestPromotorNoticia.operation = 'obtener';
...
function processAjaxPromotorNoticia() {
    ...
    if (
        ...
        json = {
            idnoticia: (noticiaSelected == undefined ? 0 : noticiaSelected.idnoticia),
            titulo: (noticiaSelected == undefined ? "" : noticiaSelected.titulo),
            descripcion: (noticiaSelected == undefined ? "" : noticiaSelected.descripcion)
        };
        let dataImagen = $("#txtNoticiaInicio").prop("files")[0];
        form_data.append("txtNoticiaInicio", dataImagen);
