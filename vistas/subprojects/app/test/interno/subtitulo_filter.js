...
function toListSubtituloC(beanPagination) {
  ...
  SubtituloSelected = beanPagination.list[0];
...
`;
  ...
  if (testSelected != undefined) {
    ...
    document.querySelector('#txtSubtitulo > option[value="' + beanPagination.list[0].codigo + '"]').setAttribute('selected', 'selected');
  }
...
function findBySubtituloC(idsubTitulo) {
  ...
  beanPaginationSubtituloC.list.forEach((subtitulo) => {
    if (idsubTitulo == subtitulo.codigo) {
