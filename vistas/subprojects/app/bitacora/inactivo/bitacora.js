...
function processAjaxBitacora() {
    ...
    switch (beanRequestBitacora.operation) {
        ...
        default:
            let fecha = getDateJava(removeDays(new Date(), 0)).split("/");
            parameters_pagination +=
                '?filter=' + document.querySelector("#txtSearchCliente").value.trim();
            parameters_pagination +=
                '&estado=' + fecha[0] + "-" + fecha[1] + "-" + fecha[2];
            parameters_pagination +=
    ...
    $.ajax({
        ...
        if (beanCrudResponse.messageServer !== null) {
            if (beanCrudResponse.messageServer.toLowerCase() == 'ok') {
                ...
                document.querySelector("#sizePageBitacora").value = 5;
...
function listaBitacora(beanPagination) {
    ...
    if (beanPagination.list.length == 0) {
        ...
        row += `<tr>
        <td class="text-center" colspan="9">NO HAY ALUMNOS INACTIVOS</td>
        </tr>`;
...
<button class="btn btn-warning ver-bitacora" >INACTIVO</button>
</td>
<td class="text-center">
