...
function processAjaxLibro() {
	...
	$.ajax({
		url:
			getHostAPI() +
			beanRequestLibro.entity_api +
			'/' +
			beanRequestLibro.operation +
			parameters_pagination,
		...
		.done(function (beanCrudResponse) {
			...
			if (beanCrudResponse.messageServer !== null) {
				if (beanCrudResponse.messageServer.toLowerCase() == 'ok') {
					...
					document.querySelector('#sizePageLibro').value = 10;
...
function addLibro(libro = undefined) {
	...
	document.querySelector('#txtCodigoLibro').value =
		libro == undefined ? '' : libro.codigo;

	document.querySelector('#txtNombreLibro').value =
		libro == undefined ? '' : libro.nombre;

	if (libro !== undefined) {
		$('#imagePreview').html(
			`<img width='150' alt='user-picture' class='img-responsive center-box' src='${getHostFrontEnd()}adjuntos/libros/${
				libro.imagen
			}' />`
		);
	} else {
...
function listaLibro(beanPagination) {
	...
	document.querySelector('#titleManagerLibro').innerHTML = ' LIBRO';
...
<td class="text-center ver-capitulo aula-cursor-mano" style="width:10%;"><img src="${getHostFrontEnd()}adjuntos/libros/${
			libro.imagen
		}" alt="user-picture" class="img-responsive center-box" width="100%"></td>
...
function addEventsButtonsLibro() {
	document.querySelectorAll('.ver-capitulo').forEach((btn) => {
		...
		btn.onclick = function () {
			libroSelected = findByLibro(btn.parentElement.getAttribute('idlibro'));
			...
			if (libroSelected != undefined) {
				...
				console.log('warning => ', 'No se encontró el libro para poder ver');
			}
		};
	});
...
function findIndexLibro(idbusqueda) {
	return beanPaginationLibro.list.findIndex((Libro) => {
		if (Libro.idlibro == parseInt(idbusqueda)) return Libro;
	});
}

function findByLibro(idlibro) {
	return beanPaginationLibro.list.find((Libro) => {
...
var validarDormularioLibro = () => {
	...
	if (beanRequestLibro.operation == 'add') {
		...
		if (
			!(
				document.querySelector('#txtImagenLibro').files[0].type ==
					'image/png' ||
				document.querySelector('#txtImagenLibro').files[0].type ==
					'image/jpg' ||
				document.querySelector('#txtImagenLibro').files[0].type == 'image/jpeg'
			)
		) {
			showAlertTopEnd(
				'info',
		...
		if (document.querySelector('#txtImagenLibro').files.length != 0) {
			...
			//menor a   1700 KB
			if (
				document.querySelector('#txtImagenLibro').files[0].size >
				1700 * 1024
			) {
				showAlertTopEnd(
					'info',
