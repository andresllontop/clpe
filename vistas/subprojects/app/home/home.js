...
function processAjaxMensaje() {
	...
	$.ajax({
		url:
			getHostAPI() +
			beanRequestMensaje.entity_api +
			'/' +
			beanRequestMensaje.operation +
...
function listaMensaje(beanPagination) {
	beanPagination.list.forEach((mensaje) => {
		if (mensaje.countMensaje) {
			...
			document.querySelector('#countAlumnoActivo').innerHTML =
				mensaje.countAlumnoActivo;
		} else if (mensaje.countAlumnoEstadoActivo) {
			document.querySelector('#countAlumnoEstadoActivo').innerHTML =
				mensaje.countAlumnoEstadoActivo;
		} else if (mensaje.countAlumnoResumen) {
			document.querySelector('#countAlumnoEstadoActivo2').innerHTML =
				mensaje.countAlumnoResumen;
		} else if (mensaje.countTarea) {
			document.querySelector('#countTarea').innerHTML = mensaje.countTarea;
		} else {
			document.querySelector('#countCitas').innerHTML = mensaje.countCita;
		}
	});
}
